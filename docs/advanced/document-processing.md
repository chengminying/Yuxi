...
Yuxi 将上传文件先保存为原文件，再解析为 Markdown 并按知识库分块策略入库。管理员可在“设置 → OCR 配置”选择默认 OCR 方法，并维护确有需要的服务地址和云端凭证；知识库上传或临时附件解析仍可逐次选择方法，未显式选择时使用系统默认项。文档状态、存储 Owner 和检索链路见[知识库机制详解](../mechanisms/knowledge-base.md)。
...
| MinerU Official | 复杂文档 | 无 | 官方云服务，无本地部署步骤 |
