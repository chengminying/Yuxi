...
- 优化品牌首页布局：统一内容宽度与响应式网格，协调顶栏、首屏及能力卡片区的高度和留白，并移除页脚年份和版本号。
- 知识库与工作区的单文件上传上限由 100 MB 提升至 500 MB，CLI 知识库上传保持相同限制。
- 新增 0.6.2 企业数据库无损迁移：启动时幂等桥接用户、知识库、对话、运行、消息反馈与 MCP 旧字段，补齐 0.7.1 必需列，并保留知识库嵌入及大模型配置。
- MinIO 宿主机端口支持通过 `MINIO_API_PORT/MINIO_CONSOLE_PORT` 配置，默认使用 `9100/9101`，容器内部端口保持不变以避开 Windows 保留端口。
- 企业定制分支同步 GitHub `upstream/main` 最新代码：保留维盈/中国电信江西工业互联网研究院品牌首页、登录页与静态资源，继续要求 API/worker 等待 Neo4j 健康；MinerU 改用上游 `vLLM 0.11.2 + MinerU 3.4.4` 固定版本实现，不再携带旧 AIMv2 运行时补丁，并保持解析服务仅绑定本机端口；Docker 构建上下文排除本地测试资料、演示数据与 pnpm 缓存。
